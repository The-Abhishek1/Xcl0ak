...
export default function NewsPage() {
  const { data: news, loading, refetch } = useLiveFetch<any[]>("/api/news", [], 120000); // refresh every 2 min
  const [sel, setSel] = useState<any>(null);
  const [fRegion, setFRegion] = useState("All");
  const [fCat, setFCat] = useState("All");

  const filtered = news.filter((n: any) =>
    (fRegion === "All" || n.region === fRegion) &&
  ...
  if (sel) return (
    <div className="animate-fade-in">
      <button onClick={() => setSel(null)} className="inline-flex items-center gap-1.5 text-sm text-brand-500 hover:underline mb-5"><ArrowLeft size={14} /> Back to News</button>
      <div className="xc-card p-6">
        ...
        <div className="flex gap-4 text-sm text-[var(--text-muted)] mb-6">
          ...
          {sel.sourceUrl && (
            <a href={sel.sourceUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-info hover:underline">
        ...
        <div className="text-sm text-[var(--text-secondary)] leading-relaxed whitespace-pre-wrap mb-6">{sel.content}</div>
        {sel.cves?.length > 0 && (
        ...
        {sel.tags?.length > 0 && (
          <div className="flex gap-1.5 flex-wrap mt-3">
            {sel.tags.map((t: string) => <Tag key={t}>{t}</Tag>)}
          </div>
        )}
      </div>
    </div>
    ...
    <div className="animate-fade-in">
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex items-center gap-2">
          <span className="text-[10px] font-mono text-[var(--text-muted)] flex items-center gap-1"><Rss size={10} className="text-brand-500" /> Live RSS feeds</span>
      ...
      <div className="flex gap-1.5 mb-2 flex-wrap">
        {REGIONS.map(r => (
          <button key={r} onClick={() => setFRegion(r)} className={`px-3 py-1.5 rounded-xl text-[11px] font-semibold border transition ${fRegion === r ? "bg-brand-500/8 text-brand-500 border-brand-500/20" : "text-[var(--text-muted)] border-[var(--border)] hover:bg-[var(--bg-tertiary)]"}`}>{r}</button>
        ))}
      </div>
      ...
      <div className="flex gap-1.5 mb-4 flex-wrap">
        {["All", ...NEWS_CATEGORIES].map(c => (
          <button key={c} onClick={() => setFCat(c)} className={`px-2.5 py-1 rounded-lg text-[10px] font-semibold border transition ${fCat === c ? "bg-purple-500/8 text-purple-600 dark:text-purple-400 border-purple-500/20" : "text-[var(--text-muted)] border-[var(--border)] hover:bg-[var(--bg-tertiary)]"}`}>{c}</button>
        ))}
      </div>

      <div className="text-xs text-[var(--text-muted)] mb-3">
        {loading ? <span className="flex items-center gap-1"><Loader2 size={11} className="animate-spin" /> Fetching live news feeds...</span>
          : <span>{filtered.length} articles from RSS feeds</span>}
      </div>
          ...
          {filtered.map((n: any) => (
            <button key={n.id} onClick={() => setSel(n)} className="xc-card p-4 text-left hover:shadow-card-hover dark:hover:shadow-card-dark transition-all group">
              <div className="flex justify-between items-start mb-2">
                ...
                <span className="text-[9px] font-mono text-[var(--text-muted)] shrink-0">{timeAgo(n.publishedAt)}</span>
              </div>
              ...
              <div className="flex justify-between items-center">
                ...
                <div className="flex gap-1.5">
                  <Tag color="#06b6d4">{n.country}</Tag>
                  <Tag color="#8b5cf6">{n.category}</Tag>
                </div>
              </div>
            </button>
          ))}
